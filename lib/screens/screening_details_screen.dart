...
class ScreeningDetailsScreen extends StatelessWidget {
  ...
  void _shareReport() {
    ...
    if (p != null) {
      ...
      buffer.writeln('Smoking: ${p.smokingStatus}');
    }
    buffer.writeln('------------------------------------------');
    buffer.writeln('STATUS: ${session.status.label.toUpperCase()}');
    buffer.writeln('RISK CATEGORY: ${r.riskCategory.toUpperCase()} (Score: ${r.riskScore}/100)');
    buffer.writeln('------------------------------------------');
    buffer.writeln('VITALS & SENSORS:');
    buffer.writeln('• SpO2: ${session.vitals.spo2 > 0 ? '${session.vitals.spo2}%' : 'Not recorded'}');
    buffer.writeln('• Heart Rate: ${session.vitals.heartRate > 0 ? '${session.vitals.heartRate} bpm' : 'Not recorded'}');
    buffer.writeln('• Airflow Pressure: ${session.sensorReading?.pressure != null ? '${session.sensorReading!.pressure!.toStringAsFixed(2)} kPa' : 'Not recorded'}');
    buffer.writeln('• Cough Activity: ${session.acoustic.coughCount > 0 ? '${session.acoustic.coughCount} coughs' : 'Not recorded'}');
    buffer.writeln('------------------------------------------');
  ...
  Widget build(BuildContext context) {
    ...
    final score = session.riskScore ?? r.riskScore;
          ...
          children: [
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  children: [
                    _buildDetailRow('Full Name', p?.fullName ?? 'Not recorded'),
                    ...
                    const SizedBox(height: 6),
                    _buildDetailRow('Smoking Status', p?.smokingStatus ?? 'Not recorded'),
                  ],
                ),
              ),
            ),
            ...
            // Sensor Measurements
            const Text('Vitals & Sensor Readings', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: AppTheme.textLight)),
            const SizedBox(height: 8),
            Row(
              children: [
                _buildSensorBox('SpO2 Saturation', session.vitals.spo2 > 0 ? '${session.vitals.spo2}%' : 'Not recorded', Icons.water_drop, AppTheme.primaryTeal),
                const SizedBox(width: 10),
                _buildSensorBox('Heart Rate', session.vitals.heartRate > 0 ? '${session.vitals.heartRate} bpm' : 'Not recorded', Icons.favorite, Colors.redAccent),
              ],
            ),
              ...
              children: [
                _buildSensorBox(
                  ...
                  session.sensorReading?.pressure != null
                      ? '${session.sensorReading!.pressure!.toStringAsFixed(2)} kPa'
                      : (session.spirometry.pefLpm > 0 ? '${session.spirometry.pefLpm.toStringAsFixed(0)} L/m' : 'Not recorded'),
                  Icons.air,
                ...
                _buildSensorBox(
                  ...
                  session.sensorReading?.coughActivity != null
                      ? '${(session.sensorReading!.coughActivity! * 100).toStringAsFixed(0)}%'
                      : (session.acoustic.coughCount > 0 ? '${session.acoustic.coughCount} coughs' : 'Not recorded'),
                  Icons.mic,
  ...
  Widget _buildSensorBox(String title, String val, IconData icon, Color color) {
    return Expanded(
      child: Container(
